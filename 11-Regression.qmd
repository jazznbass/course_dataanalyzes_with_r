...
$dist_i = -17.579 + 3.932 \times speed_i$

## Use `summary()` for detailed analyses {.smaller}
...
## Categorical predictors: <br> comparing groups
...
-   Sum contrasts for two factor levels:\

``` r
contrasts(mtcars$am_factor) <- contr.sum(2)
```
...
-   Set the contrasts of the factor to *Sum* and calculate the model.
...
mtcars$wt_centered <- scale(mtcars$wt, scale = FALSE)
contrasts(mtcars$transmission) <- contr.sum(2)
...
```{r}
wmisc::nice_regression_table(fit1)
```

##  {.smaller}

```{r}
wmisc::nice_regression_table(fit1, fit2)
```

##  {.smaller}

```{r}
wmisc::nice_regression_table(
  fit1, fit2,
  labels_models = c("Model with sum contrast", "Model with treatment contrast"),
  rename_cols = list(SE = "se", "Estimate" = "B")
)
```
...
Level 2:\
$\beta_{0j} = \gamma_{01}G_j + \upsilon_{0j}$\
...
$\gamma_{00}$ = Intercept\
$G$ = Level 2 predictor (grouping variable)\
$\gamma_{01}$ = Weight for $G$\
