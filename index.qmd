...
abstract: Here you find the presentation slides and further files for my introductory course in R
format: html
execute:
  echo: false
warning: false
...
for (i in seq_along(files)) {
  ...
  cat(sprintf("| %s | <a href=\"%s\" target=\"_blank\" rel=\"noopener\">Open</a> | <a href=\"%s\" download>Download</a> |\n",
              ttl, rel, rel))
}
...
res_dir <- "docs/exercises"
if (dir.exists(res_dir)) {
  res_files <- list.files(res_dir, pattern = "\\.html$", recursive = TRUE, full.names = TRUE)
  if (length(res_files)) {
    cat("\n\n# Exercises\n\n")
    rel <- gsub("^docs/", "", res_files)
    for (r in rel) {
