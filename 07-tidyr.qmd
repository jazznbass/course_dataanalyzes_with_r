...
```{r}
#| echo: FALSE
df_wide <- data.frame(player=c('A', 'B', 'C', 'D', 'E', 'F'),
                 year1=c(12, 15, 19, 19, 14, 17),
                 year2=c(22, 29, 18, 12, 24, 20))
df_wide
```
:::
...
We want to turn this wide format into a long format like this: 
...
-   Load the dataset `excercise.csv` from the coursepage:
...
```{r echo=FALSE}
df_student <- read.csv(file.path("files", "students.csv"))
df_student[1:100,]
```
...
```{r echo=FALSE}
df_teacher <- read.csv(file.path("files", "teachers.csv"))
df_teacher
```
...
full_join(df_student, df_teacher, by = "class_id", suffix = c("_student", "_teacher")) |> head(100)
```
...
-   Load the student and teacher datasets from the course page:
