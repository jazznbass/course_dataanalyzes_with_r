...
## Create a new variable in a dataset with missing values {.smaller}
...
## Create new variables from existing ones {.smaller}
...
::: tip
Use the `trunc` function or the `%%` modulo and the integer division `%/%` operators. Use the help function if needed)
:::
