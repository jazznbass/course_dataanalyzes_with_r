...
::: task
-   Take the `heights` dataset from the `dslabs` library. (Hint: `install.packages("dslabs")`)
...
## Shortcut for simple count tables {.smaller}
...
- Create a cross table depicting: The percentage of females and males that are above and below a height of 170 cm.
- Hint 1: `ifelse(height * 2.54 <= 170, "small", "large")`\
- Hint 2:\
  `small / (small + large) * 100`\
  `large / (small + large) * 100`\
...
heights %>%
  mutate(category = ifelse(height * 2.54 <= 170, "small", "large")) %>% 
  count(sex, category) %>%
  pivot_wider(names_from = "category", values_from = "n") %>%
  mutate(
    "% <= 170" = small / (small + large) * 100,
    "% > 170" = large / (small + large) * 100,
...
## Tables with dplyr: For values of a third variable {.smaller}
...
::: task
-   Take the `storms` dataset (from the dpylr package) and calculate n and mean of the wind speed (`wind`) by month (`month`) and storm classification (`status`) in a cross-table.
:::
...
# Inferential statistics {background-image="images/bg-h1-3.jpg" .smaller}
