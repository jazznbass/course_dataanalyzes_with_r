...
- Basic structure:  ***dependent variable ~ explanatory variables***  
(e.g. ***y ~ x1 + x2***)
...
fit1 <- lm(mpg ~ wt + transmission + wt:transmission, data = mtcars)
summary(fit1)
contrasts(mtcars$transmission) <- contr.treatment(2)
fit2 <- lm(mpg ~ wt + transmission + wt:transmission, data = mtcars)
summary(fit2)
...
library(sjPlot)
tab_model(fit1, fit2,
  show.ci = FALSE, show.se = TRUE,string.se = "se", string.est = "B",dv.labels = c("Model with helmert contrast", "Model with treatment contrast")
)
```
