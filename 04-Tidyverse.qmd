...
- Basic knowledge on the pipe `%>%` `|>` operators.
...
- Although **tidyverse** is *loved* by many, there are also critics to it [worth reading](https://matloff.github.io/TidyverseSkeptic/Skeptic.html).
- Tidyverse can be installed through R (´install.packages("tidyverse")`).
:::

# Pipes: `%>%` or `|>` {background-image="images/bg-h1-3.jpg"}
...
- Round all values to the first decimal (Note: `round()` is your friend here).
...
## The base R pipe: |> {.smaller}

- A similar pipe operator `|>` is now included in base R (since R 4.1.0).
- It works similar to `%>%` but has some limitations.
- Rule: with base R code, use `|>`, with tidyverse code, use `%>%`.
- Example:
```{r eval = FALSE}
mtcars |>
  select(mpg, cyl) |>
  head()
```
- More info: [R-bloggers: The base R pipe operator](https://www.r-bloggers.com/2021/05/the-base-r-pipe-operator/)
- [R documentation: Pipe](https://rdrr.io/r/base/pipe.html)
