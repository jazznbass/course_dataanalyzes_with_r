...
## Basic concepts of ggplot2 {.smaller}
...
-   Plot a graph with ...
    -   `cty` (city miles per gallon) and `hwy` (highway miles per galon) displayed on the axis and
...
```{r ggplot-ex-1}
ggplot(mpg, aes(x = cty, y = hwy, colour = class, shape = drv))  + 
  geom_point()
```
...
starwars %>% 
  ...
  ggplot(aes(x = species, y = mean_bmi)) +
    geom_col() +
    theme(axis.text.x = element_text(angle = 40, hjust=1))
```
...
## Distributions of multiple datapoints in categories {.smaller}
...
```{r}
#| label: three-geom-point-alternatives
#| layout-ncol: 3
#| fig-width: 3
#| fig-height: 4
#| echo: false
...
::: task
- Recreate this plot and add fitting title and axis labels.
:::
````
ggplot(mpg,aes(x = displ, y = hwy)) + 
  geom_point() +
...
ggplot(mpg,aes(x = displ, y = hwy)) +
  ...
  labs(x = "Displacement (liters)", y = "Highway Miles per Gallon")
```
...
#| code-fold: true
#| code-summary: "Code for this example"
...
ggplot(mpg, aes(x = class, y = 235.2 / hwy)) +
  ...
  ylim(5,20) +
  ylab("Liters per 100 kilometers on a highway")
```
